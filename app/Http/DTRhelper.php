...
function getPending($id,$userid = null)
{
	...
	if($id == 5)
	{
		$ctr = App\Request_leave::whereNotNull('parent_leave')->where('user_id',$empid)->where('leave_id',$id)->whereIn('leave_action_status',['Pending','Approved'])->whereNull('process_code')->get()->sum('leave_deduction');
	}
	else
	{
		//SINGLE DATE
		$ctr_0 = App\Request_leave::whereNull('parent_leave')->whereIn('leave_deduction',[1,0.5])->where('user_id',$empid)->where('leave_id',$id)->whereIn('leave_action_status',['Approved','Pending'])->whereNull('process_code')->get()->sum('leave_deduction');

		//MULTIPLE DATES
		$ctr_1 = App\Request_leave::where('parent','YES')->whereIn('leave_deduction',[1,0.5])->where('user_id',$empid)->where('leave_id',$id)->whereIn('leave_action_status',['Approved','Pending'])->whereNull('process_code')->get()->sum('leave_deduction');
...
function getPending2($id,$userid)
{
	$i = "";
	if($userid == null)
	{
	...
	{
		//MULTIPLE DATES
		$ctr_0 = App\Request_leave::whereNull('parent_leave')->where('user_id',$empid)->where('leave_id',$id)->whereIn('leave_action_status',['Approved','Pending'])->whereNull('process_code')->get()->sum('leave_deduction');

		$ctr_0_text = App\Request_leave::whereNull('parent_leave')->where('user_id',$empid)->where('leave_id',$id)->whereIn('leave_action_status',['Approved','Pending'])->whereNull('process_code')->get();

		foreach ($ctr_0_text as $key => $value) {
			$i .= $value->id.",";
		}
		...
		$ctr_1_text = App\Request_leave::where('parent','YES')->whereIn('leave_deduction',[1,0.5])->where('user_id',$empid)->where('leave_id',$id)->whereIn('leave_action_status',['Approved','Pending'])->whereNull('process_code')->get();
...
function getDTRemp($d,$user_id,$employment_id,$empcode)
{
	...
	switch ($employment_id) {
		...
		case 7:
		case 8:
				$dtr = App\Employee_icos_dtr::select('id','fldEmpDTRamIn','fldEmpDTRamOut','fldEmpDTRpmIn','fldEmpDTRpmOut','fldEmpDTRotIn','fldEmpDTRotOut','dtr_remarks','wfh','fldEmpDTRdate','dtr_option_id')->whereDate('fldEmpDTRdate',$d)->where('user_id',$user_id)->orderBy('id','DESC')->first();
		...
		default:
				...
				$dtr = App\Employee_dtr::select('id','fldEmpDTRamIn','fldEmpDTRamOut','fldEmpDTRpmIn','fldEmpDTRpmOut','fldEmpDTRotIn','fldEmpDTRotOut','dtr_remarks','wfh','fldEmpDTRdate','dtr_option_id')->whereDate('fldEmpDTRdate',$d)->where('fldEmpCode',$empcode)->orderBy('id','DESC')->first();
				...
				{
					return array("id" => "","fldEmpDTRamIn" => "","fldEmpDTRamOut" => "","fldEmpDTRpmIn" => "","fldEmpDTRpmOut" => "","fldEmpDTRotIn" => "","fldEmpDTRotOut" => "", "dtr_ot" => "","dtr_remarks" => "","dtr_request" => "","wfh" => null,"dtr_to" => "","fldEmpDTRdate" => "","dtr_option_id" => 6,"request" => "");
				}
			break;
	}	
}
...
function getLeave2($userid,$mon,$yr)
    {
        ...
        $collection = collect($ctr = App\Request_leave::whereNotNull('parent')->where('user_id',$userid)->whereMonth('leave_date_from',$mon)->whereYear('leave_date_from',$yr)->whereIn('leave_action_status',['Approved','Processed','Disapproved'])->get());
...
function getWeekSchedStaff($userid,$dt)
{
	...
	{
		return "<span style='cursor:pointer' onclick='addSched(".$userid.",\"".$dt."\")'>-</span>";
	}
}
...
function showDate(Array $dtr,$dt,$i,$dayDesc,$userid,$employmentid,$username,$lates,$under)
{
	...
	// {
	// 	$leave2 = getLeaveDetails($dt,$userid);
	// 	$leave2 = $leave2['leave_deduction_time'];
	// }
	// else
	// {
	// 	$leave2 = "NONE";
	...
	// return "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td align='center' colspan='8' class='text-success'><b>".$leave."</b></td></tr>";
									...
									if(checkIfHasLeave($dt,$userid))
											{
												...
												// if($leave['lwop'] == 'YES')
													// $lwop = " (LWOP)";
												// return "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td align='center' colspan='8' class='text-success'><b>".$leave2."</b></td></tr>";
												switch($leave['leave_deduction_time'])
												{
													case "wholeday":
														$rows = "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td align='center' colspan='8' class='text-success'><b>".$leave['leave_desc']."$lwop</b></td><td></td></tr>";
													break;
													...
													case "AM":
														...
														if($lst)
														{
															...
															{
																$rows = "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td align='center' colspan='2' class='text-success'><b>".$leave['leave_desc']."$lwop</b></td><td colspan='2' align='center'><b>WFH</b></td><td></td><td></td><td></td><td></td><td></td></tr>";
															}
															else
															{
																$rows = "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td align='center' colspan='2' class='text-success'><b>".$leave['leave_desc']."$lwop</b></td><td colspan='2' align='center'>SKELETON WF<td><td></td><td></td><td></td><td></td><td></td></tr>";
															}
														}
														else
														{
															$rows = "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td align='center' colspan='2' class='text-success'><b>".$leave['leave_desc']."$lwop</b></td><td>".formatTime($dtr['fldEmpDTRpmIn'])."</td><td>".formatTime($dtr['fldEmpDTRpmOut'])."</td><td></td><td></td><td></td><td></td><td></td></tr>";
														}
													...
													if($lst)
													{
														if($lst['wfh'] != null || $lst['wfh'] != "")
														{
															$rows = "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td colspan='2' align='center'><b>WFH</b></td><td align='center' colspan='2' class='text-success'><b>".$leave['leave_desc']."$lwop</b></td><td></td><td></td><td></td><td></td><td></td></tr>";
														}
														else
														{
															$rows = "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td colspan='2' align='center'>SKELETON WF<td><td align='center' colspan='2' class='text-success'><b>".$leave['leave_desc']."$lwop</b></td><td></td><td></td><td></td><td></td><td></td></tr>";
														}
													}
													else
													{
														$rows = "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td>".formatTime($dtr['fldEmpDTRpmIn'])."</td><td>".formatTime($dtr['fldEmpDTRpmOut'])."</td><td align='center' colspan='2' class='text-success'><b>".$leave['leave_desc']."$lwop</b></td><td></td><td></td><td></td><td></td><td></td></tr>";
													}
											...
											{
												$rows =  "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td align='center' colspan = '8'>SKELETON WF</td><td>".$remarks."</td></tr>";
											}
								}
								else
								{
									$rows =  "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td align='center' style='width:10%'></td><td align='center' style='width:10%'></td><td align='center' style='width:10%'></td><td align='center' style='width:10%'></td><td align='center' style='width:10%'></td><td align='center'></td><td align='center'></td><td></td><td align='center'>".$remarks."</td></tr>"; 
								}
											...
											switch($leave['leave_deduction_time'])
											{
												...
												case "AM":
													...
													if($lst)
													{
														if($lst['wfh'] != null || $lst['wfh'] != "")
														{
															$rows = "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td colspan='2' align='center'><b>WFH</b></td><td align='center' colspan='2' class='text-success'><b>".$leave['leave_desc']."</b></td><td></td><td></td><td></td><td></td><td></td></tr>";
														}
														else
														{
															$rows = "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td colspan='2' align='center'>SKELETON WF<td><td align='center' colspan='2' class='text-success'><b>".$leave['leave_desc']."</b></td><td></td><td></td><td></td><td></td><td></td></tr>";
														}
													}
													else
													{
														$rows = "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td>".formatTime($dtr['fldEmpDTRpmIn'])."</td><td>".formatTime($dtr['fldEmpDTRpmOut'])."</td><td align='center' colspan='2' class='text-success'><b>".$leave['leave_desc']."</b></td><td></td><td></td><td></td><td></td><td></td></tr>";
													}
						...
						switch($leave['leave_deduction_time'])
						{
							...
							case "AM":
														...
														if($lst)
														{
															if($lst['wfh'] != null || $lst['wfh'] != "")
															{
																$rows = "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td align='center' colspan='2' class='text-success'><b>".$leave['leave_desc']."$lwop</b></td><td colspan='2' align='center'><b>WFH<b><td></td><td></td><td></td><td></td><td></td></tr>";
															}
															else
															{
																$rows = "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td align='center' colspan='2' class='text-success'><b>".$leave['leave_desc']."$lwop</b></td><td colspan='2' align='center'>SKELETON WF<td><td></td><td></td><td></td><td></td><td></td></tr>";
	...
	{
		$rows = "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td align='center' colspan='8' class='text-success'><b>".getHoliday($dt)."</b></td><td></td></tr>";
	}
	...
	if($list)
	{
		if(!checkIfHasLeave($dt,$userid))
		{
			...
			{
				case "Wholeday":
					$rows = "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td align='center' colspan='8' class='text-success'><b>WFH</b></td><td></td></tr>";
				break;
				...
				case "AM":
					...
					$rows = "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td align='center' colspan='2' class='text-success'><b>WFH</b></td><td align='center' colspan='2'>SKELETON WF</td><td></td><td></td><td></td><td></td><td></td></tr>";
				break;
				...
				case "PM":
					...
					$rows = "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td align='center' colspan='2'>SKELETON WF</td><td align='center' colspan='2' class='text-success'><b>WFH</b></td><td></td><td></td><td></td><td></td><td></td></tr>";
			   break;
			}
		}
		else
	...
	if($tos)
	{
		...
		{
			case "wholeday":
				$rows = "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td align='center' colspan='7' class='text-success'><b>On-Trip</b></td><td></td><td></td></tr>";
			break;
			...
			case "AM":
				...
				$rows = "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td align='center' colspan='2' class='text-success'><b>On-Trip</b></td><td colspan='2' align='center'>SKELETON WF</td><td></td><td></td><td></td><td></td><td></td></tr>";
			break;
			...
			case "PM":
				...
				if($lst)
				{
					if($lst['wfh'] != null || $lst['wfh'] != "")
					{
						$rows = "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td colspan='2' align='center'><b>WFH</b></td><td align='center' colspan='2' class='text-success'><b>On-Trip</b></td><td></td><td></td><td></td><td></td><td></td></tr>";
					}
					else
					{
						$rows = "<tr><td style='width:10%'><span>".$i."</span><span style='float:right'>".$dayDesc."</span></td><td colspan='2' align='center'>SKELETON WF</td><td align='center' colspan='2' class='text-success'><b>On-Trip</b></td><td></td><td></td><td></td><td></td><td></td></tr>";
					}
				}
				else
				{
...
function getPrevDTR($type,$empcode)
{
	$dtr = App\DTRProcessed::where('empcode',$empcode)->orderBy('id','DESC')->first();
