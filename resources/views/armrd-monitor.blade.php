...
<body>
	...
	<?php 
    //GET ALL STAFF
    $emp = getAllStaffDivision3('I');

  ?>
  <h2>
    <form method="POST" id="frm" enctype="multipart/form-data" role="form" action="{{ url('monitor-armrd/EzUt1cg19i') }}">  
    {{ csrf_field() }}
    <select name="userid" id="userid" class="form-control input-lg" onchange="submitFrm()">
      @foreach($emp AS $emps)
        <option value="{{ $emps->id }}">{{ strtoupper($emps->lname.', '.$emps->fname) }}</option>
      @endforeach
    </select>
  </form>
  </h2>
