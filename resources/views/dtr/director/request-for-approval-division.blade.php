...
<h5 class="alert alert-warning"><center>This screen display the staff's filed leave applications, travel order and overtime requests. For your immediate action.</center></h5>
<br>
<div class="row">
  <div class="col-12">
      <div class="card card-primary card-outline card-outline-tabs">
              <div class="card-header p-0 border-bottom-0">
                <ul class="nav nav-tabs" id="custom-tabs-three-tab" role="tablist">
                  ...
                  <li class="nav-item">
                    <a class="nav-link" id="custom-tabs-three-messages-tab3" data-toggle="pill" href="#custom-tabs-three-messages3" role="tab" aria-controls="custom-tabs-three-messages3" aria-selected="false">Disapproved</a>
                  </li>
              ...
              <div class="card-body">
                <div class="tab-content" id="custom-tabs-three-tabContent">
                            ...
                            @else
                              @if($lists->director == 'NO')
                                <td>
                                  ...
                                  @if($lists->leave_action_status == 'Pending')
                                    ...
                                    @elseif($lists->leave_action_status == 'Processed' || $lists->leave_action_status == 'Disapproved')
                                      <span class="fas fa-print text-blue" style="cursor:pointer;" onclick="toPrint('{{ $lists->leave_desc }}',{{ $lists->id }})"></span>
                                    @elseif($lists->leave_action_status == 'Monetized')
                                ...
                                @else
                                  <td align="center"><span class="text-muted">
                                    <span class="fas fa-print text-blue" style="cursor:pointer;" onclick="toPrint('{{ $lists->leave_desc }}',{{ $lists->id }})"></span>
                                    <span class="fas fa-trash text-red" style="cursor:pointer;" onclick="toAction('leave',{{ $lists->id }},'Cancelled')"></span>
                                  </td>
                                @endif
                              @endif
                          </tr>
                     ...
                     <?php $i = 0; ?>
                        ...
                        @if($lists->ot_status != 'Cancelled')
                          <tr>
                            ...
                            <td>
                              <center>
                                  ...
                                  @if($lists->ot_status == 'Pending')
                                        <span class="fas fa-check-circle text-green" style="cursor:pointer;" onclick="modalOT({{ $lists->id }},'Approved')"></span>
                                      ...
                                      @elseif($lists->ot_status == 'Approved' || $lists->ot_status == 'Processed')
                                        <span class="fas fa-times-circle text-red" style="cursor:pointer;" onclick="toAction('OT',{{ $lists->id }},'Cancelled')"></span>
                                        <span class="fas fa-print text-blue" style="cursor:pointer;" onclick="toPrint('Overtime',{{ $lists->id }})"></span>
                            ...
                            @else
                              <td align="center"><span class="text-muted">
                                @if($lists->ot_status == 'OED Approved')
                                        <span class="fas fa-check-circle text-green" style="cursor:pointer;" onclick="modalOT({{ $lists->id }},'Time Edited','{{ $lists->cto }}','YES')"></span>
                                        <span class="fas fa-times-circle text-red" style="cursor:pointer;" onclick="toAction('OT',{{ $lists->id }},'Cancelled')"></span>
                                        <span class="fas fa-print text-blue" style="cursor:pointer;" onclick="toPrint('Overtime',{{ $lists->id }})"></span>
                                @else
                                <span class="fas fa-print text-primary" style="cursor:pointer;" onclick="toPrint('Overtime',{{ $lists->id }})"></span>
                                <span class="fas fa-trash text-red" style="cursor:pointer;"></span>
                                @endif
                              </td>
                            @endif
                  ...
                  <div class="tab-pane fade" id="custom-tabs-three-messages3" role="tabpanel" aria-labelledby="custom-tabs-three-messages-tab3">
                    <br>
                     <table id="tbl5" class="table table-bordered table-striped">
                      <thead>
                      <tr>
                        <!-- <th style="width: 2%">
                          <input type="checkbox" id="icheck_all" name="icheck_all" class="check" value="all">
                  ...
                  <div class="tab-pane fade" id="custom-tabs-three-messages4" role="tabpanel" aria-labelledby="custom-tabs-three-messages-tab4">
                    <br>
                     <table id="tbl6" class="table table-bordered table-striped">
                      <thead>
                      <tr>
                        <!-- <th style="width: 2%">
                          <input type="checkbox" id="icheck_all" name="icheck_all" class="check" value="all">
                      ...
                      <?php
                        foreach(getApproved('leave',Auth::user()->division) AS $v2)
                        {
                          ...
                          if($user)
                          {
                            ...
                            echo "<tr><td>".$user['lname'].", ".$user['fname']." ".$user['mname']."</td><td>".$lv['leave_desc']."</td><td>".$dt."</td><td>".$v2->leave_deduction_time."</td><td>".date('M d, Y H:i:s',strtotime($v2->created_at))."</td>";
                            echo "<td align='center' style='width: 10%'><span class='fas fa-times-circle text-danger' style='cursor:pointer;' onclick='toAction(\"leave\",".$v2->id.",\"Cancelled\")'></span> 
                        ...
                        foreach(getApproved('to',Auth::user()->division) AS $v3)
                        {
                          ...
                          if($user)
                          {
                            ...
                            echo "<tr><td>".$user['lname'].", ".$user['fname']." ".$user['mname']."</td><td>Travel Order</td><td>".$dt2."</td><td>".$v3->to_deduction_time."</td><td>".date('M d, Y H:i:s',strtotime($v3->created_at))."</td>";
                        ...
                        foreach(getApproved('ot',Auth::user()->division) AS $v4)
                        {
                          ...
                          if($user)
                          {
                            ...
                            echo "<tr><td>".$user['lname'].", ".$user['fname']." ".$user['mname']."</td><td>Overtime</td><td>".$dt3."</td><td>".$v4->ot_deduction_time."</td><td>".date('M d, Y H:i:s',strtotime($v4->created_at))."</td>";
...
<form method="POST" id="frm_request_action">
  ...
  <input type="hidden" name="request_ot_director" id="request_ot_director" value="NO">
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <strong>TIME-IN</strong>
                      <br>
                      <p class="text-muted">
                        <input type="time" class="form-control" name="cto_in" id="cto_in" min="17:31:00" value="17:31:00">
                      </p>
              <strong>TIME-OUT</strong>
                      <br>
                      <p class="text-muted">
                        <input type="time" class="form-control" name="cto_out" id="cto_out" min="17:32:00" value="17:32:00">
                      </p>
      ...
      <div class="modal fade" id="modal-batch-ot">
        <div class="modal-dialog modal-sm">
          <div class="modal-content">
            ...
            <b>DATE</b><br>
            <input type="date" class="form-control" name="dt_ot" required>
            </form>
            </div>
            <div class="modal-footer justify-content-between">
              <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
...
function toAction(type,id,act) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        // else if(type == 'ot')
        // {
        //   action = "{{ url('dtr/request-action-ot') }}";
        // }
        // else
        // {
...
function modalOT(id,act,cto,director = null)
{
  //GET TIME
